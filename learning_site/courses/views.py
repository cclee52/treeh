...
def course_list(request):
    courses = models.Course.objects.all().annotate(total_steps=Count('text', distinct=True)+Count('quiz', distinct=True))
    total = courses.aggregate(total=Sum('total_steps'))
    email = 'questions@learning_site.com'
    return render(request, 'courses/course_list.html', {'courses': courses, 'total': total, 'email': email})
